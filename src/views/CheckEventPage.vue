...
type EventRuleSection =
    ...
    const sections: readonly EventRuleSection[] = [
  {
    ...
    title: "一、活動辦法",
    preamble:
      "加入官方LINE「新北市政府體育局」，進入【LINE傳聖火抽黃金】專屬頁面、建立或接受朋友邀請加入隊伍，並完成指定打卡任務就有機會參加抽獎，將好禮帶回家！",
    items: [
      "活動期間：2026.05.01 至 2026.05.22 23:59止",
      "活動說明：本活動以個人LINE帳號組隊參加，於本活動頁面點選建立或接受朋友邀請加入隊伍，每位隊員填寫相關資訊並同意加入活動隊伍後，即為成員之一，每位參加者僅可加入一支隊伍。每支隊伍須有５名隊員，即可開始進行LINE傳聖火活動。",
      "本活動分為二條路線任務【1】全臺22縣市聖火傳遞【2】新北市29行政區聖火傳遞。每條路線任務於活動期間僅可執行1次，完成該路線任務即可獲得1次抽獎機會，二條路線任務獎項內容各有不同，歡迎踴躍參加！",
    ],
  },
  {
    kind: "ruleParts",
    ...
    parts: [
      {
        ...
        items: [
          "活動期間內，５位參與隊員可至全臺22縣市指定打卡點，依照本活動打卡流程請與店面招牌或指定字樣合影(本人臉部正面須入鏡) ，依步驟上傳照片，並出現［傳遞完成］頁面即代表完成此地點打卡。",
          "每一縣市聖火傳遞打卡完成，該縣市地圖即變色象徵點亮聖火、傳遞打卡完成。",
          "每一隊員須至少完成一縣市聖火傳遞打卡，且隊員間不能重複縣市，全隊須累積完成５個不同縣市之打卡點。",
          "每一隊伍只要完成全臺5個不同縣市聖火傳遞打卡，即獲得抽奬1次資格。",
        ],
      },
      {
        heading: "路線二：新北市29行政區聖火傳遞",
        items: [
          "活動期間內，５位參與隊員可至新北市29行政區指定打卡點，依照本活動打卡流程請與店面招牌或指定字樣合影(本人臉部正面須入鏡) ，依步驟上傳照片，並出現［傳遞完成］頁面即代表完成此地點打卡。",
          "每一行政區聖火傳遞打卡完成，該行政區地圖即變色象徵點亮聖火、傳遞打卡完成。",
          "每一隊員須至少完成一區域聖火傳遞打卡，且隊員間不能重複區域，全隊須累積完成10個不同行政區之打卡點。",
          "每一隊伍只要完成新北市10個不同行政區聖火傳遞打卡，即獲得抽奬1次資格。",
        ],
      },
    ],
  },
  {
    ...
    title: "三、注意事項",
    items: [
      "本活動僅限已設定手機門號的用戶透過LINE帳號參加，每個LINE帳號限為一人、單一使用。若發生系統無法正常運作的情形，請將LINE應用程式升級為最新版本。",
      "每位參加者僅能建立或參加一支活動隊伍，參加隊伍後即無法修改、退出、建立其他隊伍。",
      "單一路線每位隊員皆須進行聖火傳遞打卡，且須達成各路線規定條件即符合1次抽獎資格。",
      "每一隊員至指定傳遞點打卡，須與店面招牌或指定字樣（如：85度C、澎湖縣馬公北辰市場、連江縣南竿福澳港、坪林老街等）合影(本人臉部正面須入鏡) ，並依步驟上傳照片與活動指定圖框合成後，出現［傳遞完成］頁面即代表完成此地點打卡。",
      "本活動指定傳遞點係依合作店家營運資訊規劃，惟因門市營運狀況可能隨時調整（包含但不限於歇業、遷址、新設等），主辦單位保留隨時修正、調整活動據點之權利。",
      "本活動截止時間以本活動官方帳號系統主機時間為準。",
      "參加者參與本活動，即同意將照片使用權授權給主辦單位使用。",
      "嚴禁AI生成，僅接受真實拍攝，請於拍攝時開啟定位並於裝置內保存聖火傳遞打卡照片之原始檔至活動獎項公布，若有獲獎則會依原始檔進行資格查核，得獎者須配合辦理相關中獎後之查驗及領獎依據作業；進行聖火傳遞打卡前，請確認拍攝照片與本活動辦法及規定相符。",
      "登入註冊隊伍時，中華民國國民請登真實完整的中文名字、手機號碼及電子郵件帳號(E-mail)，姓名必須與身分相關證明文件所載內容一致；在臺外籍人士須登錄真實完整的姓名、手機號碼、電子郵件帳號(Email)，姓名必須與護照上所載內容一致，並保證所登錄資料均為真實且正確，如有不實、資料不全或錯誤導致無法聯絡通知者或中獎機會損失等，主辦單位得取消中獎人中獎資格，且如有致損害於主辦、執行單位或其他任何第三人，參加者應負一切相關責任。",
      "違規處理：參加者若以惡意之電腦程式或其他明顯違反活動正常運行之方式，意圖混淆或影響活動結果者，一經察覺，主辦單位得立即取消參加、抽獎及得獎資格，並得追回獎品。參加者因違反相關規定所產生之一切法律責任，需自行負擔。",
      "本活動抽獎品項與數量，依本活動相關公告為準。各項獎品以實物為主，圖片僅供參考。活動獎項限中獎者本人領取，不得轉贈或販售予第三人，活動獎項無法指定規格類型，得獎者亦不得要求折抵現金、退換、轉售或折換其他物品，主辦單位保有修改或變更活動方式及更換活動獎品權利且不得要求折換現金，若中獎者無法配合活動辦法領取則視同放棄，無法請求其他權利或補償。",
      "參加活動即同意若因個人裝置、網路設備、環境等因素導致活動無法順利完成，主辦單位及LINE將不負擔任何責任或損害賠償。",
      "參加本活動應瞭解並同意提供個人資料予主辦單位，主辦單位將遵循「個人資料保護法」之規定，於本活動之特定目的範圍內蒐集、處理及利用您的個人資料。",
      "實體獎之價值總額超過20,000元者，經核實中獎資格後，主辦機關將另與中獎人約定領獎方式，並請配合主辦機關指定之日期及地點進行領取。本活動獎項寄送地區僅限臺、澎、金、馬之可送達地區，請恕無法寄送至海外地區。",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "蒐集個人資料單位：新北市政府體育局。",
      "蒐集之目的：作為參與本活動抽獎活動及後續相關程序之聯繫以及本單位推廣行銷使用，包含資料庫建立、使用者行為分析與行銷。",
      "個人資料之類別：姓名、地址、身分證資料、電子郵件、電話及台端依據活動辦法所提供予主辦單位之個人資料。",
      "個人資料利用之期間：自台端參加本活動之日起至得獎公布後三個月內，法律另有規定者則不在此限。",
      "個人資料利用之地區：中華民國境內及主辦單位業務或服務所及的其他地區、利用對象所在的地區。",
      "個人資料利用之對象：由主辦單位該業務承辦人員於辦理該活動之特定目的（含通知、公佈及獎項寄送等）必要範圍內，依通常作業所必要之方式利用此個人資料。本活動公開之資料，公眾將可透過網際網路瀏覽參與活動所公開之資料或中獎資訊。",
      "個人資料利用之方式：書面、電子文件、電話、傳真、以自動化機器或其他非自動化之利用方式。",
      "依個人資料保護法第三條規定當事人得行使之權利及方式：台端得以書面或電子郵件方式向本活動承辦單位三立電視股份有限公司提出申請（E-mail客服信箱：10010901@mail.sanlih.com.tw，回覆服務時間週一~週五，10:30~12:00、13:30~17:00，例假日及國定假日暫不提供服務），請求查詢、閱覽、製給複製本；或補充／更正、請求停止蒐集、處理、利用或刪除個人資料內容之一部分或全部。（註：台端申請查詢、閱覽、製給複製本時，將酌收必要成本費用。）前述申請，主辦單位得向台端請求提出可資確認之身分證明文件；若委託他人代為申請者，並應出具委任書，且提供本人及代理人之身分證明文件。惟依法主辦單位因執行職務所必須者，得不依本活動參與者請求為之。",
  ...
  <main class="mx-auto min-h-screen w-full max-w-[393px] bg-[#f2f0f4] px-3 pt-[max(10px,env(safe-area-inset-top))] pb-6 text-[#222]">
    ...
    <section class="mt-3 space-y-4">
      ...
      >
        ...
        <div v-else-if="section.kind === 'paragraphs'" class="mt-2 space-y-2">
          <p
